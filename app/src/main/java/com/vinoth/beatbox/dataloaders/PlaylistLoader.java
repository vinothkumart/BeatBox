...
package com.vinoth.beatbox.dataloaders;
...
public class PlaylistLoader {
    ...
    public static List<Playlist> getPlaylists(Context context, boolean defaultIncluded) {
        ...
        if (mCursor != null && mCursor.moveToFirst()) {
            do {
                ...
                final int songCount = BeatboxUtils.getSongCountForPlaylist(context, id);
    ...
    private static void makeDefaultPlaylists(Context context) {
        ...
        /* Last added list */
        final Playlist lastAdded = new Playlist(BeatboxUtils.PlaylistType.LastAdded.mId,
                resources.getString(BeatboxUtils.PlaylistType.LastAdded.mTitleId), -1);
        mPlaylistList.add(lastAdded);

        /* Recently Played */
        final Playlist recentlyPlayed = new Playlist(BeatboxUtils.PlaylistType.RecentlyPlayed.mId,
                resources.getString(BeatboxUtils.PlaylistType.RecentlyPlayed.mTitleId), -1);
        mPlaylistList.add(recentlyPlayed);

        /* Top Tracks */
        final Playlist topTracks = new Playlist(BeatboxUtils.PlaylistType.TopTracks.mId,
                resources.getString(BeatboxUtils.PlaylistType.TopTracks.mTitleId), -1);
