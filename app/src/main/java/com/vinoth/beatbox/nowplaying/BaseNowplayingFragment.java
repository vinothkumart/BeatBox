...
package com.vinoth.beatbox.nowplaying;
...
import com.vinoth.beatbox.MusicService;
import com.vinoth.beatbox.R;
import com.vinoth.beatbox.activities.BaseActivity;
import com.vinoth.beatbox.adapters.BaseQueueAdapter;
import com.vinoth.beatbox.adapters.SlidingQueueAdapter;
import com.vinoth.beatbox.dataloaders.QueueLoader;
import com.vinoth.beatbox.listeners.MusicStateListener;
import com.vinoth.beatbox.timely.TimelyView;
import com.vinoth.beatbox.utils.BeatboxUtils;
import com.vinoth.beatbox.utils.Helpers;
import com.vinoth.beatbox.utils.NavigationUtils;
import com.vinoth.beatbox.utils.PreferencesUtility;
import com.vinoth.beatbox.utils.SlideTrackSwitcher;
import com.vinoth.beatbox.widgets.CircularSeekBar;
import com.vinoth.beatbox.widgets.DividerItemDecoration;
import com.vinoth.beatbox.widgets.PlayPauseButton;
import com.vinoth.beatbox.widgets.PlayPauseDrawable;
...
public class BaseNowplayingFragment extends Fragment implements MusicStateListener {
        ...
        public void run() {
            ...
            if (mProgress != null) {
                ...
                if (elapsedtime != null && getActivity() != null)
                    elapsedtime.setText(BeatboxUtils.makeShortTimeString(getActivity(), position / 1000));
            }
            overflowcounter--;
    ...
    public Runnable mUpdateElapsedTime = new Runnable() {
        ...
        public void run() {
            if (getActivity() != null) {
                String time = BeatboxUtils.makeShortTimeString(getActivity(), MusicPlayer.position() / 1000);
                if (time.length() < 5) {
    ...
    public void setSongDetails(View view) {
        ...
        if (playPauseFloating != null) {
            playPauseDrawable.setColorFilter(BeatboxUtils.getBlackWhiteColor(accentColor), PorterDuff.Mode.MULTIPLY);
    ...
    public void updateSongDetails() {
        ...
        if (!duetoplaypause) {
            if (albumart != null) {
                ImageLoader.getInstance().displayImage(BeatboxUtils.getAlbumArtUri(MusicPlayer.getCurrentAlbumId()).toString(), albumart,
        ...
        if (songduration != null && getActivity() != null)
            songduration.setText(BeatboxUtils.makeShortTimeString(getActivity(), MusicPlayer.duration() / 1000));
